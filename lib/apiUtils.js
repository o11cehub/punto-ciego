...
export async function callAI({ system, messages, maxTokens = 2000 }) {
  ...
  const response = await fetch(API_URL, {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        ...
        // responder, y esos tokens de razonamiento salen del mismo
        // presupuesto. Con un límite bajo el modelo gasta todo pensando
        // y devuelve vacío, que se ve como 'formato inesperado'.
        maxOutputTokens: maxTokens,
        // Gemini tiene modo JSON nativo: en vez de solo pedirlo en el
        // prompt, el modelo queda obligado a devolver JSON válido.
  ...
  // Gemini puede devolver 200 sin contenido usable: si el filtro de
  // seguridad bloqueó la respuesta, o si se quedó sin tokens mientras
  // razonaba. Logueamos finishReason porque es lo que distingue un caso
  // del otro, y sin eso el error es imposible de diagnosticar.
  ...
  if (!texto) {
    console.error(
      'Gemini no devolvió texto. finishReason:',
